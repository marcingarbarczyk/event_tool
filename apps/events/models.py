...
from apps.dynamoforms.models import Form, UserAnswers
from apps.events.utils import generate_unique_code
from utils.mail import send_message_via_email
from utils.models import ActiveMixin, Orderable
...
class Bundle(models.Model):
    """
    The `Bundle` class represents a bundle
    """
...
class EventBundle(Orderable, ActiveMixin):
    ...
    def active_price(self):
        now = timezone.now()
        price = self.prices.active().filter(activity_range__contains=now).first()
        return price

    @property
    def active_discounts(self):
        return self.discounts.active()

    def __str__(self):
        return f'#{self.event_id} {self.bundle}'
    ...
    class Meta(Orderable.Meta):
        ...
        constraints = [
            UniqueConstraint(
                fields=['event', 'bundle', 'is_active'],
                name='unique event bundle',
            )
        ]
...
class EventBundlePrice(Orderable, ActiveMixin):
    ...
    class Meta(Orderable.Meta):
        constraints = [
            ExclusionConstraint(
                name='exclude overlapping activity range',
                expressions=[
                    (
                        'activity_range',
...
class EventBundleCodeDiscount(ActiveMixin):
    ...
    discount = models.DecimalField(
        default=0,
        verbose_name=_('discount in PLN'),
        max_digits=7,
        decimal_places=2,
    )
...
class Event(models.Model):
    ...
    show_no_available_places_info = models.BooleanField(
        default=False,
        verbose_name=_('show message when no available places'),
    )
    ...
    def active_bundles(self):
        return self.event_bundles.active()
...
class Registration(models.Model):
    ...
    # Previously, forms were submitted from Google Forms and landed in this field.
